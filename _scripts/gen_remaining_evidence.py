...
def gen_elc_risk_assessment():
    ...
    print("Created: ELC-004_全社リスクアセスメント結果_2025年度.xlsx")
...
def gen_elc_internal_audit_plan():
    ...
    pdf.table_row(["子会社監査（東北）", "デモA東北", "10月"], [60, 60, 60], fill=True)
    pdf.table_row(["子会社監査（タイ）", "Demo-A (Thailand)", "2月"], [60, 60, 60])
    pdf.ln(5)
...
def gen_fcrp_consolidation():
    ...
    subs = [
        ("デモA東北", date(2026, 1, 10), date(2026, 1, 9),
         "エラーなし", "なし", date(2026, 1, 9), "完了"),
        ("デモA物流サービス", date(2026, 1, 10), date(2026, 1, 10),
         "エラー1件（科目コード誤り）", "あり(往復1回)", date(2026, 1, 12), "完了"),
        ("Demo-A Thailand", date(2026, 1, 12), date(2026, 1, 11),
         "エラーなし", "なし", date(2026, 1, 11), "完了"),
        ("デモAトレーディング", date(2026, 1, 10), date(2026, 1, 10),
