...
def fix_fcrp_002():
    ...
    # PASS_AFTER_CORRECTION の件を対象: Q1 DA-LOG, DATR; Q2 DA-TB, DA-LOG; Q3 DA-TB

    corrections = [
        {'q': 'Q1', 'sub': 'DA-LOG', 'sub_name': 'デモA物流サービス', 'errs': 1,
        ...
        {'q': 'Q1', 'sub': 'DATR', 'sub_name': 'デモAトレーディング', 'errs': 1,
         'upload_ts': '2025-07-08 13:00:00', 'error_items': ['為替レート不一致 (DAT向け子会社間取引)'],
         'correspondence': [
             ('2025-07-08 13:20', '経理部課長→DATR 後藤', '送信', 'タイバーツ換算レートがグループ規定と不一致。確認願います。'),
             ('2025-07-08 14:50', 'DATR 後藤→経理部課長', '返信', 'ご指摘ありがとうございます。4.05円/THBに修正し再送します。'),
             ('2025-07-08 16:20', 'DATR→S05', '再送', '為替レート修正版を再アップロード'),
             ('2025-07-08 16:40', '経理部課長', '確認', 'レート整合確認 - PASS')]},
        {'q': 'Q2', 'sub': 'DA-TB', 'sub_name': 'デモA東北', 'errs': 2,
         'upload_ts': '2025-10-08 09:00:00', 'error_items': ['勘定科目マスタ不整合 (新規勘定)', '内部利益消去額算定誤り'],
         'correspondence': [
             ('2025-10-08 09:30', '経理部課長→DA-TB 大橋', '送信', '2件エラー: 勘定科目2211未登録 + 内部利益消去の計算方式ズレ。'),
             ('2025-10-08 11:20', 'DA-TB 大橋→経理部課長', '返信', '1件目は科目マスタ連携不具合(情シス連絡済)。2件目は計算式を原価率ベースに修正。'),
             ('2025-10-08 13:40', '経理部課長→DA-TB', '再送依頼', '両件修正の上、再送願います。'),
             ('2025-10-08 15:30', 'DA-TB→S05', '再送', '2件修正版再アップロード'),
             ('2025-10-08 15:55', '経理部課長', '確認', '修正確認 - PASS')]},
        {'q': 'Q2', 'sub': 'DA-LOG', 'sub_name': 'デモA物流サービス', 'errs': 1,
        ...
        {'q': 'Q2', 'sub': 'DATR', 'sub_name': 'デモAトレーディング', 'errs': 0,  # actually Q2 DATR is PASS not PAC, skip
         'upload_ts': '', 'error_items': [], 'correspondence': []},
        {'q': 'Q3', 'sub': 'DA-TB', 'sub_name': 'デモA東北', 'errs': 1,
         'upload_ts': '2026-01-08 15:00:00', 'error_items': ['固定資産除却仕訳の科目誤り'],
         'correspondence': [
             ('2026-01-08 15:20', '経理部課長→DA-TB 大橋', '送信', '12月除却仕訳の科目が誤り(特別損失→営業外費用)。確認願います。'),
             ('2026-01-08 16:40', 'DA-TB 大橋→経理部課長', '返信', '判断誤りです。特別損失に修正します。'),
             ('2026-01-08 17:40', 'DA-TB→S05', '再送', '科目修正の上再アップロード'),
...
def fix_fcrp_004():
    ...
    entries_by_q = {
        'Q1': [
            {'仕訳№': 'CNS-Q1-001', '区分': '投資と資本の相殺', '借方科目': '資本金', '貸方科目': '関係会社株式', '金額(円)': 300000000, '摘要': 'デモA東北の投資相殺', '起票': '自動'},
            ...
            {'仕訳№': 'CNS-Q1-005', '区分': '少数株主損益', '借方科目': '少数株主損益', '貸方科目': '利益剰余金', '金額(円)': 8520000, '摘要': 'デモAトレーディング 少数株主持分', '起票': '自動'},
            {'仕訳№': 'CNS-Q1-006', '区分': '内部利益消去(在庫)', '借方科目': '売上原価', '貸方科目': '棚卸資産', '金額(円)': 42800000, '摘要': '親→東北間在庫の未実現利益消去', '起票': '手動'},
        ],
        'Q2': [
            {'仕訳№': 'CNS-Q2-001', '区分': '投資と資本の相殺', '借方科目': '資本金', '貸方科目': '関係会社株式', '金額(円)': 300000000, '摘要': 'デモA東北の投資相殺', '起票': '自動'},
            ...
            {'仕訳№': 'CNS-Q2-005', '区分': '少数株主損益', '借方科目': '少数株主損益', '貸方科目': '利益剰余金', '金額(円)': 8520000, '摘要': 'デモAトレーディング 少数株主持分', '起票': '自動'},
            {'仕訳№': 'CNS-Q2-006', '区分': '内部利益消去(在庫)', '借方科目': '売上原価', '貸方科目': '棚卸資産', '金額(円)': 42800000, '摘要': '親→東北間在庫の未実現利益消去', '起票': '手動'},
        ],
        'Q3': [
            {'仕訳№': 'CNS-Q3-001', '区分': '投資と資本の相殺', '借方科目': '資本金', '貸方科目': '関係会社株式', '金額(円)': 300000000, '摘要': 'デモA東北の投資相殺', '起票': '自動'},
            ...
            {'仕訳№': 'CNS-Q3-005', '区分': '少数株主損益', '借方科目': '少数株主損益', '貸方科目': '利益剰余金', '金額(円)': 8520000, '摘要': 'デモAトレーディング 少数株主持分', '起票': '自動'},
            {'仕訳№': 'CNS-Q3-006', '区分': '内部利益消去(在庫)', '借方科目': '売上原価', '貸方科目': '棚卸資産', '金額(円)': 42800000, '摘要': '親→東北間在庫の未実現利益消去', '起票': '手動'},
        ],
        'Q4': [
            {'仕訳№': 'CNS-Q4-001', '区分': '投資と資本の相殺', '借方科目': '資本金', '貸方科目': '関係会社株式', '金額(円)': 300000000, '摘要': 'デモA東北の投資相殺', '起票': '自動'},
            ...
            {'仕訳№': 'CNS-Q4-005', '区分': '少数株主損益', '借方科目': '少数株主損益', '貸方科目': '利益剰余金', '金額(円)': 8520000, '摘要': 'デモAトレーディング 少数株主持分', '起票': '自動'},
    ...
    nonstd = [
        ...
        ('CNS-Q4-007', 'Q4', '期末為替換算調整', 24500000, 'DAT(タイ)の期末為替レート改定影響(3.80→4.05円/THB)', 'Q4のみ発生', '適正 - 換算レート変更反映', ACC_MGR),
    ]
